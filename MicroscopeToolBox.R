...
microscope.get.design = function(
    ...
    if(length(F) != length(D)){
        print("There is a problem, there should be one short description/code for each folder provided")
    }
    ...
    if( ! dir.exists ( DIR.res ) ){
        print("Error: the directory given for the results does not exist")
    }
    ...
    if( sum(grepl("well",names(microscope.design$PDEF))) == 0){
        print("Error: a column should be 'well' -- perhaps you also need to check the case, e.g., Well will not work")
    }
    for(each.ch in CHANELS){
        if( sum(grepl(each.ch,names(microscope.design$PDEF))) == 0 ){
            print(paste("Error: the channel ",each.ch," is given in the experiment design but does not appear in the plate definition file", sep=""))
        }
    }
    
    ## What is in each well, it has to be the same for all plates
    print("Reading csv file, make sure it is not opened on any computer ... ")    
    microscope.design$PDEF    = read.csv(PDEF)  ##
    # microscope.design$PDEF    = microscope.design$PDEF[, which(colnames(microscope.design$PDEF) %in% c("well","GFP","RFP","BFP"))]
    ...
    for( each.folder in F){
        ...
        #nd.file = list.files(path=each.folder, pattern=".nd")
        if(length(nd.file) > 1){
            print(paste("Problem: there is more than one nd file in", each.folder, " (",length(nd.file),")" ))
        }
...
microscope.load.data = function(design){
    ...
    for(each.plate in design$F){
        ...
        print(paste("Processing plate ",each.plate))
        ...
        print(paste("Loading plate ",each.plate,", there are ",length(S.positions)," positions"));
        ...
        while(FILE.NOT.FOUND & file.num < length(S.positions) ){
            ...
            res.file.name = paste(design$OUT[K],"/",design$BASENAMES[K],"s",S.positions[file.num],"Results.txt",sep="")

            if( file.exists(res.file.name) ){                
                res.file = read.csv(res.file.name, sep="\t", row.names=c(1))
                print(paste("It found a non empty file (",res.file.name,") and the number of columns is ",NCOL(res.file)))
                res.file = res.file[1,]
        ...
        if( file.num == length(S.positions)){
            print(paste("Error in plate ",each.plate,": all the result files were empty, there is no data to load"))
        }
        ...
        for( N in 1:length(S.positions) ){
            ...
            if(N %% 100 == 0){
                print(paste("Processing position number ",N))
            }
        ...
        for(L in 1:length(data[[K]])){
            ...
            while(NROW(data[[K]][[L]])>0 & not.converged & max.cycles > 0){
                ...
                if(length(to.remove)>0){
                    ...
                    print(paste("In plate ",K,"and well ",L, " the picture(s) ",paste(to.remove,",")," was/were removed"))
                } else {                    
                    not.converged=0
                }
        ...
        for(L in 1:length(data[[K]])){
            ...
            if(NROW(data[[K]][[L]])>2){
                ...
                if(length(low.fluo)>0){
                    ...
                    print(paste("In plate ",K,"and well ",L, " the picture(s) ",paste(pics.to.remove,",")," was/were removed"))
...
uscope.process.get.one.mean = function(design, data, plateNUM){
    ...
    for ( each.CH in design$CHANELS){
        ...
        if( length(my.col)==0 | length(my.col) > 1){
            if(length(my.col)==0){
                print(paste("Error: no column was found to match the chanel name", each.CH))
            } else {
                print(paste("Error: more than one column was found to match the chanel name", each.CH))
            }
        }        
